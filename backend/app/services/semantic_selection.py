...
class SemanticSelectionService:
    ...
    def _score_capability(
        ...
        combined_tokens = name_tokens | description_tokens | semantic_tokens | context_tokens
        ...
        semantic_bonus = 0.0
        if semantic_tokens:
            semantic_tokens_expanded = self._expand_tokens(semantic_tokens)
            semantic_overlap = query_tokens_expanded & semantic_tokens_expanded
            semantic_ratio = len(semantic_overlap) / len(query_tokens_expanded)
            semantic_bonus = min(0.14, len(semantic_overlap) * 0.03)
